...
public class InquiryListController extends HttpServlet {
	...
	protected void doHandler(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("p_board 접근 테스트");
		                    ...
		                    for(int i = 1; i <= lastPage2; i++) {
		                       			if(section >1 && i == 1) {
		                       				data += "<li class='page-item '>"
						                        	+"<a class='page-link' id='prev' name='prev' onclick=p_board_page(" + (section -1) +"," + i + "," +  map.get("product_num") + "," + 2 +") > 이전 </a>"
						                     	+"</li>";
		                       			};
				                	...
				                	if(i == pageNum) {
				                		 ...
				                		 data +="<li class='page-item  disabled' aria-current='page'>"
							                      	+"<a class='page-link' id='${page}' onclick=p_board_page(" +  (section) +"," + i + "," +  map.get("product_num")+")>" + ((section-1)*10 + i) + "</a>"
							                     +"</li>";
				                	}else {
				                		data +=  "<li class='page-item '>"
					   	                    	+"<a class='page-link' id='${page}' onclick=p_board_page(" +  (section) +"," + i + "," +  map.get("product_num") +")>" + ((section-1)*10 + i) + "</a>"
		                    			...
		                    			data += "<li class='page-item'>"
		        			                  	+"<a class='page-link' id='next' name='next' onclick=p_board_page(" + (section +1) +"," + 1 + "," +  map.get("product_num") + "," + 2 +") > 다음 </a>"
		      			                      +"</li>";
		                    			}
