...
public class ReviewListController extends HttpServlet {
	...
	protected void doHandler(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("review 접근 테스트");
		...
		map.put("product_num",req.getParameter("product_num"));  //매개변수 전달 필요
		map.put("section", Integer.toString(section));
		map.put("pageNum", Integer.toString(pageNum));
		
		int p_board_tot = shop_Count_Service.Count_review(map);
		...
		List<Shop_Write_ReviewDTO> review_list = shop_Select_Service.Select_Review(map);
		 ...
		 for(int i = 0; i < 4; i++) {
			 ...
			 jsonObject.put("articleNo", review_list.get(i).getArticleNo());
			 jsonObject.put("user_id", review_list.get(i).getUser_id());
			 jsonObject.put("product_num", review_list.get(i).getProduct_num());
			 jsonObject.put("content", review_list.get(i).getContent());
		                    ...
		                    for(int i = 1; i <= lastPage2; i++) {
		                       			if(section >1 && i == 1) {
		                       				data += "<li class='page-item '>"
						                        	+"<a class='page-link' id='prev' name='prev' onclick=review_page(" + (section -1) +"," + i + "," +  map.get("product_num") + "," + 2 +") > 이전 </a>"
						                     	+"</li>";
		                       			};
				                	...
				                	if(i == pageNum) {
				                		 ...
				                		 data +="<li class='page-item  disabled' aria-current='page'>"
							                      	+"<a class='page-link' id='${page}' onclick=review_page(" +  (section) +"," + i + "," +  map.get("product_num")+")>" + ((section-1)*10 + i) + "</a>"
							                     +"</li>";
				                	}else {
				                		data +=  "<li class='page-item '>"
					   	                    	+"<a class='page-link' id='${page}' onclick=review_page(" +  (section) +"," + i + "," +  map.get("product_num") +")>" + ((section-1)*10 + i) + "</a>"
		                    			...
		                    			data += "<li class='page-item'>"
		        			                  	+"<a class='page-link' id='next' name='next' onclick=review_page(" + (section +1) +"," + 1 + "," +  map.get("product_num") + "," + 2 +") > 다음 </a>"
		      			                      +"</li>";
		                    			}
