...
public class ReviewInsertController extends HttpServlet {
	...
	Map<String, String > map = new HashMap<>();
		...
		if(chk_insert == 0) {  //리뷰 쓴 기록이 없을 때
			...
			int chk = shop_Insert_Service.Write_review(shop_Write_ReviewDTO);
			...
			if(chk == 1) { //리뷰 쓰기 권한이 없을 때
				...
				String data = 
				 "<div class='m-3'>"
	            +"<div class='row'>"
	               + "<input type='text' class='review_text' value='"+ shop_Write_ReviewDTO.getContent() +"'readonly>"
	             ...
	             + "</div>"
	             + "<div class='mt-3'>"
	               + "<input type='text' value='"+shop_Write_ReviewDTO.getUser_id() +"' class='review_text' style='font-size: 0.8em; color: gray;' readonly>"
