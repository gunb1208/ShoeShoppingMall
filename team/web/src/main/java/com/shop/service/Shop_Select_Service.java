...
public enum Shop_Select_Service {
	...
	public List<Shop_Write_ProductDTO> Select_Product_Brand(Map<String, String> map) {  //브랜드별 상품 정보 가져오기 

		 modelMapper = MapperUtil.INSTANCE.get();
		 session  = MybatisManager.getInstance().openSession();
		 
		
		List<Shop_Write_ProductVO> vo = session.selectList("shop_Select.select_product_brand", map);
		List<Shop_Write_ProductDTO> dto = vo.stream().map(x -> modelMapper.map(x, Shop_Write_ProductDTO.class)).collect(Collectors.toList());
