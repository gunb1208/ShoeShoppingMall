                    ...
                    <div class="">
                        <table class="table table-borderless">
                            <thead class="table-light">
                              <tr>
                                <th class="col-1 text-center">상품이름</th>
                                <th class="col-5 text-center">상품옵션</th>
                                <th class="col-4 text-center">주문번호</th>
                            ...
                            <tbody id="order">
                                ...
                                <c:forEach var="item" items="${order_list}" step="1">
				                   ...
				                   <tr>
				                  <th scope="row">  <a href="${ctxPath}/orderView.do?order_num=${item.order_num}&user_id=${item.user_id}&product_num=${item.product_num}&product_name=${item.product_name}&product_opt=${item.product_opt}&result=${item.result}"> ${item.product_name} </a></th>
                ...
                <div class="p-2 bg-opacity-10 border border-2 border-start-0 border-end-0">
                    <div class="">
                        <table class="table table-borderless">
                            <thead class="table-light">
                              <tr>
                                <th class="col-4 text-center">문의상품</th>
                            ...
                            <tbody id ="pboard">
                             ...
                             <c:forEach var="item" items="${p_board_list}" step="1">
                              ...
                              <tr>
                                <td class="col-4"><a href="${ctxPath}/productView.do?product_name=${item.product_name}">${item.product_name} </a></td>
                                <td class="col-4">${item.qna_title}</td>
			...
			success: function(data, textStatus){
				...
				$('#order').append(
						 "<tr>"
		                   ...
		                   +"<a href='${ctxPath}/orderView.do?order_num=" 
			                + jsonInfo.list[i].order_num 
			                +"&user_id=" + jsonInfo.list[i].user_id 
			 				+ "&product_num=" + jsonInfo.list[i].product_num 
		                 ...
		                 + jsonInfo.list[i].order_num
		                 + "</td>"
		                 + "<td id='center'>" + "2020-05-30"+ "</td>"
		                 + "<td id='center'>" + jsonInfo.list[i].result + "</td>"
		               + "</tr>"
		               )
		...
		var user_id2 = $('#user_id').val()
     	...
     	$.ajax({
			...
			success: function(data, textStatus){
				...
				$('#pboard').append(
                      "<tr>"
      		        + "<td class='col-4'> <a href='${ctxPath}/productView.do?product_name="+jsonInfo.list[i].product_name + "'>" + jsonInfo.list[i].product_name+ "</a>"+ "</td>"
      		        + "<td class='col-4'>" + jsonInfo.list[i].qna_content+ "</td>"
